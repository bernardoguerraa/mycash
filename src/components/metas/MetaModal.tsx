...
export default function MetaModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      {/* Modal */}
      <div className="card animate-scale-in relative w-full max-w-lg rounded-2xl">
        {/* Header */}
        ...
        <form onSubmit={handleSubmit} className="space-y-5 p-6">
          ...
          <div>
            ...
            />
            {errors.titulo && (
              <p className="mt-1 text-xs text-rose-400">{errors.titulo}</p>
            )}
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                className={`w-full rounded-lg border bg-surface-3 px-3.5 py-2.5 text-sm text-white placeholder-zinc-500 outline-none transition-colors focus:ring-2 focus:ring-emerald-500/20 font-mono-nums ${
                  errors.valorObjetivo
                    ? 'border-rose-500'
                    : 'border-edge-2 focus:border-emerald-500/20'
                }`}
              />
              {errors.valorObjetivo && (
                <p className="mt-1 text-xs text-rose-400">{errors.valorObjetivo}</p>
              )}
            </div>
            <div>
              <label className="mb-1.5 block text-sm font-medium text-zinc-300">
              ...
              <input
                ...
                placeholder="0,00"
                className="w-full rounded-lg border border-edge-2 bg-surface-3 px-3.5 py-2.5 text-sm text-white placeholder-zinc-500 outline-none transition-colors focus:border-emerald-500/20 focus:ring-2 focus:ring-emerald-500/20 font-mono-nums"
              />
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                className={`w-full rounded-lg border bg-surface-3 px-3.5 py-2.5 text-sm text-white outline-none transition-colors focus:ring-2 focus:ring-emerald-500/20 [color-scheme:dark] ${
                  errors.dataInicio
                    ? 'border-rose-500'
                    : 'border-edge-2 focus:border-emerald-500/20'
                }`}
              />
              {errors.dataInicio && (
                <p className="mt-1 text-xs text-rose-400">{errors.dataInicio}</p>
              )}
            </div>
            <div>
              <label className="mb-1.5 block text-sm font-medium text-zinc-300">
              ...
              />
              {errors.dataLimite && (
                <p className="mt-1 text-xs text-rose-400">{errors.dataLimite}</p>
              )}
            </div>
          </div>
          ...
          <div>
            ...
            <select
              ...
              className="w-full rounded-lg border border-edge-2 bg-surface-3 px-3.5 py-2.5 text-sm text-white outline-none transition-colors focus:border-emerald-500/20 focus:ring-2 focus:ring-emerald-500/20 [color-scheme:dark]"
            >
